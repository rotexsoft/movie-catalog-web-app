...
return function(SlimApp $app, array $appSettings, ContainerInterface|null $injectedContainer =null): void {
    
    $container = $injectedContainer ?? $app->getContainer();
    ...
    */
    $app->get('/init-users/{password}[/]', function (\Psr\Http\Message\ServerRequestInterface $req, \Psr\Http\Message\ResponseInterface $resp, $args) {

        return $resp->withStatus(301)
    ...
    if( !SMVC_APP_USE_MVC_ROUTES ) {
        ...
        $app->get('/', function($request, $response, $args) {
            $response->getBody()->write("Hello World!<br>This is the default non-mvc route");
            return $response;
        });
    }
    ...
    $isLocaleObjectValid = $localeObject instanceof \Vespula\Locale\Locale;
    ...
    $isLocaleObjectValid && $logErrorRenderer->setDefaultErrorTitle($localeObject->gettext('default_application_error_title_text'));
    $isLocaleObjectValid && $logErrorRenderer->setDefaultErrorDescription($localeObject->gettext('default_application_error_title_description'));
    ...
    $isLocaleObjectValid && $jsonErrorRenderer->setDefaultErrorTitle($localeObject->gettext('default_application_error_title_text'));
    $isLocaleObjectValid && $jsonErrorRenderer->setDefaultErrorDescription($localeObject->gettext('default_application_error_title_description'));
    ...
    $isLocaleObjectValid && $xmlErrorRenderer->setDefaultErrorTitle($localeObject->gettext('default_application_error_title_text'));
    $isLocaleObjectValid && $xmlErrorRenderer->setDefaultErrorDescription($localeObject->gettext('default_application_error_title_description'));
    ...
    if($appSettings[AppSettingsKeys::DISPLAY_ERROR_DETAILS] && $isLocaleObjectValid) {
